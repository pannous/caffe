...
import matplotlib._pylab_helpers as helper
...
def key_event(event): 
  print event.key 
  if event.button==2 or event.key=='q':
    sys.exit(0)
...
##        use_marker = False
    ...
    # fig=plt.figure() 
    for fig in helper.Gcf.get_all_fig_managers():
      fig.canvas.mpl_connect('key_press_event', key_event)
      fig.canvas.mpl_connect('button_press_event', key_event)
